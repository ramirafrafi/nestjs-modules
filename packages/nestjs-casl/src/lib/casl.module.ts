...
export class CaslModule {
  static forRoot<T extends Abilities>(options: ICaslModuleOptions<T>): DynamicModule {
    return {
      ...
      providers: [
        (options.options.abilityFactory as Type).name
          ? {
              provide: CASL_ABILITY_FACTORY,
              useClass: options.options.abilityFactory as Type,
              scope: Scope.REQUEST,
            }
          : {
              provide: CASL_ABILITY_FACTORY,
              useValue: options.options.abilityFactory,
              scope: Scope.REQUEST,
            },
      ],
  ...
  static forRootAsync<T extends Abilities>(options: ICaslModuleAsyncOptions<T>): DynamicModule {
    return {
      ...
      providers: [
        {
          provide: CASL_ABILITY_FACTORY,
          useFactory: async (...args) => (await options.useFactory(args)).abilityFactory,
